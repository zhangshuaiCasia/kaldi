...
# steps/info/chain_dir_info.pl exp_train_dev_sp/chain/tdnn7q_sp
# exp_train_dev_sp/chain/tdnn7q_sp: num-iters=394 nj=3..16 num-params=18.7M dim=40+100->6034 combine=-0.058->-0.057 (over 8) xent:train/valid[261,393,final]=(-1.20,-0.897,-0.894/-1.20,-0.919,-0.911) logprob:train/valid[261,393,final]=(-0.090,-0.059,-0.058/-0.098,-0.073,-0.073)
...
dir=exp_train_dev_sp/chain/epoch_tdnn_1024_19layer${affix}${suffix}
...
train_set=train_dev_sp$suffix
ali_dir=exp_train_dev_sp/tri5a_ali$suffix
treedir=exp_train_dev_sp/chain/tree$suffix
lang=data/lang
...
if [ $stage -le 9 ]; then
  ...
  nj=$(cat exp_train_dev_sp/tri5a_ali$suffix/num_jobs) || exit 1;
  steps/align_fmllr_lats.sh --nj $nj --cmd "$train_cmd" data/$train_set \
    data/lang exp_train_dev_sp/tri5a exp_train_dev_sp/tri5a_lats$suffix
  rm exp_train_dev_sp/tri5a_lats$suffix/fsts.*.gz # save space
fi
...
if [ $stage -le 12 ]; then
  ...
  tdnnf-layer name=tdnnf2 $tdnnf_opts dim=1024 bottleneck-dim=160 time-stride=1
  tdnnf-layer name=tdnnf3 $tdnnf_opts dim=1024 bottleneck-dim=160 time-stride=1
  tdnnf-layer name=tdnnf4 $tdnnf_opts dim=1024 bottleneck-dim=160 time-stride=1
  tdnnf-layer name=tdnnf5 $tdnnf_opts dim=1024 bottleneck-dim=160 time-stride=0
  tdnnf-layer name=tdnnf6 $tdnnf_opts dim=1024 bottleneck-dim=160 time-stride=3
  tdnnf-layer name=tdnnf7 $tdnnf_opts dim=1024 bottleneck-dim=160 time-stride=3
  ...
  tdnnf-layer name=tdnnf16 $tdnnf_opts dim=1024 bottleneck-dim=160 time-stride=3
  tdnnf-layer name=tdnnf17 $tdnnf_opts dim=1024 bottleneck-dim=160 time-stride=3
  tdnnf-layer name=tdnnf18 $tdnnf_opts dim=1024 bottleneck-dim=160 time-stride=3
  tdnnf-layer name=tdnnf19 $tdnnf_opts dim=1024 bottleneck-dim=160 time-stride=3
...
if [ $stage -le 13 ]; then
  if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d $dir/egs/storage ]; then
    utils/create_split_dir.pl \
     /exp_train_dev_sport/b0{5,6,7,8}/$USER/kaldi-data/egs/swbd-$(date +'%m_%d_%H_%M')/s5c/$dir/egs/storage $dir/egs/storage
  fi

#    --cmd "queue.pl --config /home/dpovey/queue_conly.conf" \
    # --feat.online-ivector-dir exp_train_dev_sp/nnet3/ivectors_${train_set} \
  ...
  steps/nnet3/chain/train.py --stage $train_stage \
    ...
    --tree-dir $treedir \
    --lat-dir exp_train_dev_sp/tri5a_lats$suffix \
    --dir $dir  || exit 1;
...
if [ $stage -le 14 ]; then
  ...
  # the lang directory.
  utils/mkgraph.sh --self-loop-scale 1.0 data/lang_test_train_dev $dir $dir/graph
fi
...
# --online-ivector-dir exp_train_dev_sp/nnet3/ivectors_${decode_set} \
if [ $stage -le 15 ]; then
  rm $dir/.error 2>/dev/null || true
  for decode_set in test/IOS $maybe_rt03; do
      (
      steps/nnet3/decode.sh --acwt 1.0 --post-decode-acwt 10.0 \
          --nj $decode_nj --cmd "$decode_cmd" $iter_opts \
          $graph_dir data/${decode_set} \
          $dir/decode_IOS || exit 1;
      if $has_fisher; then
...
if $test_online_decoding && [ $stage -le 16 ]; then
  ...
  steps/online/nnet3/prepare_online_decoding.sh \
       ...
       $lang exp_train_dev_sp/nnet3/extractor $dir ${dir}_online
