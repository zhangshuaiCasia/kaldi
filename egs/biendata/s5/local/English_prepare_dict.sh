...
(echo SIL; echo SPN;) > $dict_dir/silence_phones.txt
echo SIL > $dict_dir/optional_silence.txt

# nonsilence phones; on each line is a list of phones that correspond
# really to the same base phone.
cat $res_dir/cmudict.0.7a.symbols | perl -ane 's:\r::; print;' | \
 perl -e 'while(<>){
...
grep -v ';;;' $res_dir/cmudict.0.7a_lower | \
 perl -ane 'if(!m:^;;;:){ s:(\S+)\(\d+\) :$1 :; print; }' \
  > $dict_dir/lexicon1_raw_nosil.txt || exit 1;
...
(echo '!SIL SIL'; echo '<SPOKEN_NOISE> SPN'; ) | \
 cat - $dict_dir/lexicon1_raw_nosil.txt | sort | uniq > $dict_dir/lexicon2_raw.txt || exit 1;


# lexicon.txt is without the _B, _E, _S, _I markers.
# This is the input to wsj_format_data.sh
cp $dict_dir/lexicon2_raw.txt $dict_dir/lexicon.txt
