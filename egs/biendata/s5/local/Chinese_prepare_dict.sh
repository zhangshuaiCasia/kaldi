...
[ $# != 2 ] && echo "Usage: $0 <resource-path>" && exit 1;

res_dir=$1
dict_dir=$2

mkdir -p $dict_dir
cp $res_dir/lexicon.txt.txt $dict_dir

cat $dict_dir/lexicon.txt.txt | awk '{ for(n=2;n<=NF;n++){ phones[$n] = 1; }} END{for (p in phones) print p;}'| \
