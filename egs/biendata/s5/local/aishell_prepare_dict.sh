...
res_dir=$1
dict_dir=data/local/dict_last
mkdir -p $dict_dir
cp $res_dir/lexicon_last.txt $dict_dir

cat $dict_dir/lexicon_last.txt | awk '{ for(n=2;n<=NF;n++){ phones[$n] = 1; }} END{for (p in phones) print p;}'| \
